                    ...
                    <form action="{{ route('tarefas.store') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea id="descricao" name="descricao" rows="4" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{{ old('descricao') }}</textarea>
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                                        ...
                                        <option value="em_andamento" {{ old('status') === 'em_andamento' ? 'selected' : '' }}>{{ __('Em andamento') }}</option>
                                        <option value="concluida" {{ old('status') === 'concluida' ? 'selected' : '' }}>{{ __('Concluída') }}</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="prazo" name="prazo" type="date" class="mt-1 block w-full" value="{{ old('prazo') }}" />
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <x-text-input id="prioridade" name="prioridade" type="number" min="1" max="5" class="mt-1 block w-full" value="{{ old('prioridade', 1) }}" required />
                                    <x-input-error :messages="$errors->get('prioridade')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="concluido_em" :value="__('Data de conclusão')" />
                                    <x-text-input id="concluido_em" name="concluido_em" type="date" class="mt-1 block w-full" value="{{ old('concluido_em') }}" />
                                    <x-input-error :messages="$errors->get('concluido_em')" class="mt-2" />
                                </div>
                            </div>

                            <div class="flex items-center gap-4">
                                <x-primary-button>{{ __('Salvar tarefa') }}</x-primary-button>
